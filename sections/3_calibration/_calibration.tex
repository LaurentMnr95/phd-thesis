\chapter{Consistenct Study of Adversarial loss}
\minitoc
...
\begin{definition}[Margin loss] We say a loss $L$ is a margin loss if and only their exist a measurable function $\phi:\mathbb{R}\to\mathbb{R}_+$, such that for all $x\in\mathcal{X}$, $y\in\mathcal{Y}$, $f:\mathcal{X}\to\mathbb{R}$ a measurable function,
$$L(x,y,f)=\phi(yf(x))$$
\end{definition}
...
\paragraph{Calibration in adversarial classification} We need to note that the notion of calibration in the adversarial setting is misleading. EXPLAIN WHY 
...
\begin{figure}
    \centering
    %\includegraphics[scale=0.6]{images/Exemple.png}
