...
\paragraph{Lipschitz Regularization of Neural Networks.} To build more robust neural networks, an emerging technique is to add a regularization $\Omega(f)$ to the classification objective to encourage a smaller Lipschitz constant. 
...
\begin{align*}
 M\times\left(I+\frac12Q+\frac38Q^2+\dots+(-1)^p\begin{pmatrix} n\\k \end{pmatrix}
Q^p+\dots \right).
\end{align*}
with $Q = I-A^TA$ 
